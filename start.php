...
function modal_info_init()
{
    ...
    elgg_register_menu_item('page', array('name' => 'modal_info', 'href' => 'modal_info/all', 'text' => elgg_echo('modal_info:all'), 'context' => 'admin', 'parent_name' => 'appearance', 'section' => 'configure'));
}
/**
...
function modal_info_page_handler($segments)
{
    ...
    switch ($section) {
        case 'add':
            ...
            return true;
        case 'edit':
            echo elgg_view('resources/modal_info/edit', array('guid' => $segments[0]));
            return true;
        case 'view':
            echo elgg_view('resources/modal_info/view', array('guid' => $segments[0]));
            return true;
        case 'all':
...
function modal_info_setup_menu($hook, $type, $return, $params)
{
    ...
    if ($entity->canEdit()) {
        $return[] = ElggMenuItem::factory(array('name' => 'edit', 'text' => elgg_echo('edit'), 'href' => "modal_info/edit/{$entity->guid}", 'priority' => 800));
        $return[] = ElggMenuItem::factory(array('name' => 'delete', 'text' => elgg_view_icon('delete'), 'href' => "actions/entities/delete?guid={$entity->guid}", 'confirm' => true, 'priority' => 900));
        return $return;
    }
}